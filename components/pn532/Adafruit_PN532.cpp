...
// static const char *TAG = "adafruit_pn532";
...
/**************************************************************************/
/*!
    @brief  Instantiates a new PN532 class using software SPI.PN532DEBUG
...
*/
/**************************************************************************/
Adafruit_PN532::Adafruit_PN532(gpio_num_t clk, gpio_num_t miso, gpio_num_t mosi, gpio_num_t ss):
  _ss(ss),
  _clk(clk),
  _mosi(mosi),
  _miso(miso),
...
*/
/**************************************************************************/
// Adafruit_PN532::Adafruit_PN532(gpio_num_t irq, gpio_num_t reset):
//   _ss((gpio_num_t) 0),
//   _clk((gpio_num_t) 0),
//   _mosi((gpio_num_t) 0),
//   _miso((gpio_num_t) 0),
...
//   gpio_set_direction(_reset, GPIO_MODE_OUTPUT);
// }
...
*/
/**************************************************************************/
Adafruit_PN532::Adafruit_PN532(gpio_num_t ss):
  _ss(ss),
  _clk((gpio_num_t) 14),
  _mosi((gpio_num_t) 13),
  _miso((gpio_num_t) 12),
...
void Adafruit_PN532::begin() {
  if (_usingSPI) {
    // SPI initialization
    if (_hardwareSPI) {
      spi_bus.init(_mosi, _miso, _clk, _ss);
  ...
  //   // Reset the PN532
  //   gpio_set_level(_reset, 1);
  //   gpio_set_level(_reset, 0);
...
*/
/**************************************************************************/
void Adafruit_PN532::PrintHexChar(const uint8_t * data, const uint32_t numBytes)
{
  uint32_t szPos;
  for (szPos=0; szPos < numBytes; szPos++)
  {
...
bool Adafruit_PN532::writeGPIO(uint8_t pinstate) {
  ...
  #ifdef PN532DEBUG
    printf("Writing P3 GPIO: "); printf("%x\n", pn532_packetbuffer[1]);
  #endif
...
bool Adafruit_PN532::setPassiveActivationRetries(uint8_t maxRetries) {
  ...
  #ifdef MIFAREDEBUG
    printf("Setting MxRtyPassiveActivation to "); printf("%d", maxRetries); printf(" \n");
  #endif
...
bool Adafruit_PN532::readPassiveTargetID(uint8_t cardbaudrate, uint8_t * uid, uint8_t * uidLength, uint16_t timeout) {
  ...
  #ifdef MIFAREDEBUG
    printf("Found "); printf("%d", pn532_packetbuffer[7]); printf(" tags\n");
  #endif
  ...
  #ifdef MIFAREDEBUG
    printf("ATQA: 0x");  printf("%x\n", sens_res);
    printf("SAK: 0x");  printf("%x\n", pn532_packetbuffer[11]);
  #endif
...
bool Adafruit_PN532::inDataExchange(uint8_t * send, uint8_t sendLength, uint8_t * response, uint8_t * responseLength) {
  ...
  if (!waitready(1000)) {
    #ifdef PN532DEBUG
      printf("Response never received for APDU...\n");
    #endif
    return false;
  }
  ...
  if (pn532_packetbuffer[0] == 0 && pn532_packetbuffer[1] == 0 && pn532_packetbuffer[2] == 0xff) {
    ...
    if (pn532_packetbuffer[5]==PN532_PN532TOHOST && pn532_packetbuffer[6]==PN532_RESPONSE_INLISTPASSIVETARGET) {
      if (pn532_packetbuffer[7] != 1) {
        ...
        #endif
        printf("Number of tags inlisted:\n");
...
uint8_t Adafruit_PN532::mifareclassic_AuthenticateBlock (uint8_t * uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t * keyData)
{
  ...
  _uidLen = uidLen;
    ...
    printf("Using authentication KEY ");printf("%c", keyNumber ? 'B' : 'A');printf(": ");
...
{
  #ifdef MIFAREDEBUG
    printf("Trying to read 16 bytes from block ");printf("%d\n", blockNumber);
  #endif
...
{
  #ifdef MIFAREDEBUG
    printf("Trying to write 16 bytes to block ");printf("%d\n", blockNumber);
  #endif
...
uint8_t Adafruit_PN532::mifareultralight_WritePage (uint8_t page, uint8_t * data)
{
  ...
  #ifdef MIFAREDEBUG
    printf("Trying to write 4 byte page");printf("%d\n", page);
  #endif
...
void Adafruit_PN532::writecommand(uint8_t* cmd, uint8_t cmdlen) {
  if (_usingSPI) {
    ...
    #endif
    gpio_set_level(_ss, 0);
    vTaskDelay(10 / portTICK_PERIOD_MS);     // or whatever the delay is for waking up the board
    ...
    #ifdef PN532DEBUG
    printf(" 0x"); printf("%x", (uint8_t)PN532_PREAMBLE);
      printf(" 0x"); printf("%x", (uint8_t)PN532_PREAMBLE);
      printf(" 0x"); printf("%x", (uint8_t)PN532_STARTCODE2);
      printf(" 0x"); printf("%x", (uint8_t)cmdlen);
      printf(" 0x"); printf("%x", (uint8_t)(~cmdlen + 1));
      printf(" 0x"); printf("%x", (uint8_t)PN532_HOSTTOPN532);
    #endif
    ...
    for (uint8_t i=0; i<cmdlen-1; i++) {
      ...
      checksum += cmd[i];
      #ifdef PN532DEBUG
        printf(" 0x"); printf("%x", (uint8_t)cmd[i]);
      #endif
    }
    ...
    #ifdef PN532DEBUG
      printf(" 0x"); printf("%x", (uint8_t)~checksum);
      printf(" 0x"); printf("%x", (uint8_t)PN532_POSTAMBLE);
      printf("\n");
    #endif
  }
  // else {
...
void Adafruit_PN532::spi_write(uint8_t c) {
  if (_hardwareSPI) {
    ...
    for (i=0; i<8; i++) {
      ...
      if (c & _BV(i)) {
        gpio_set_level(_mosi, 1);
      } else {
        gpio_set_level(_mosi, 0);
      }
      gpio_set_level(_clk, 1);
    }
  }
